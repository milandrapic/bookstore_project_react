...
//used in the login component, logs the user in
  ...
  addToCart = (book, quantity) =>{
    let newCart = [];
    console.log('in ATC method');
    console.log(book);
    this.state.cart.forEach(element => {
      newCart.push(element);
    });
    newCart.push({book:book, quantity:quantity});
    this.setState(
      {cart: newCart}
    );
  ...
  render() {
    ...
    if (this.state.authuser.authenticated) {
      todo = (
      <Browse user={this.state.authuser} cart={this.state.cart} atc={this.addToCart}  click={this.logout} />
        );
    }
